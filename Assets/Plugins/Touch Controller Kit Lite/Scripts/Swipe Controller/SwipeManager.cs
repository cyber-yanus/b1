...
public class SwipeManager : MonoBehaviour
{
    ...
    public static event TapHandler OnSingleTapLeft;
    public static event TapHandler OnSingleTapRight;
    ...
    private void TriggerSingleTap() //! Called by Function name with string reference
    {
        ...
        if (canTap)
        {
            float middleWidth = Camera.main.pixelWidth / 2f;
            float touchXPosition = Input.mousePosition.x;
            
            if (touchXPosition < middleWidth)
            {
                //left tap
                if (OnSingleTapLeft != null)
                {
                    //Calling delegate event
                    OnSingleTapLeft.Invoke();
                }
            }
            else
            {
